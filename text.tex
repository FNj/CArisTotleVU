...
Vedoucí práce mě upozornil na systém \textit{R Shiny}. Jedná se o knihovnu psanou v jazyce \textit{R}, ve kterém je psaná i samotná knihovna \texttt{catest}. Já zase přišel s nápadem použít jako jádro aplikace \uv{microframework} (aplikační rámec) \textit{Flask} psaný v \textit{Pythonu}. \cite{R} \cite{r_shiny} \cite{python} \cite{flask}

Systém \textit{R Shiny} je ovšem cílen na různé datově analytické výstupy spíše než na obecnější webové aplikace, také mám opravdu jen malé zkušenosti s programovacím jazykem \textit{R}, ve kterém je tento systém napsaný.

Raději jsem tedy nakonec zůstal u \textit{Pythonu} a \textit{Flasku}, se kterým mám jak ze školního, tak profesionálního prostředí asi největší zkušenosti ze všech programovacích jazyků. Konkrétně jsem

Protože takto používám kombinaci dvou různých programovacích jazyků, je nutné mít k dispozici nějaké rozhraní mezi nimi. Původně jsem uvažoval o úplně vlastním řešení, ale nakonec jsem našel knihovnu \textit{RPy2}, která mi tvorbu rozhraní velmi usnadnila. \cite{rpy2}

Aplikace musí ukládat dlouhodobě informace o uživatelích, definicích adaptivního testu a průběhu jednotlivých případů testování. Tyto jsou ukládány do relační databáze pomocí knihovny \textit{SQLAlchemy}, konkrétně jsem při vývoji používal pro jeho jednoduchost nasazení databázový systém \textit{SQLite}, ten je ovšem asi nejsnáze nahraditelnou částí celého systému. \cite{sqlalchemy} \cite{sqlite}

\section{Datový model}
V této sekci je krátce popsán použitý datový model. Graficky je znázorněn na obrázku \ref{fig:data_model}. Jednotlivé uvedené entity jsou vlastně zároveň tabulky databázi a třídy objektů v samotném prostředí \textit{Pythonu} ve smyslu objektově relačního zobrazení (konceptem shrnutým třeba v \cite{ORM}).
...
Tabulky \texttt{users}, \texttt{roles} a \texttt{roles\_users} slouží k zaznamenávání identit uživatelů. Kdyby existencí tabulky pro role nepředpokládala knihovna \textit{Flask-Security}, která zajišťuje v aplikaci správu identit, tak by se \texttt{roles}, samozřejmě tedy ani \texttt{roles\_users}, v modelu nevyskytla. % TODO: dát citaci na Flask-Security % TODO: zjednodušit formulaci

Tabulka \texttt{tests} obsahuje informace o testu jako celku. Sloupec \texttt{net\_definition} obsahuje definici struktury sítě i tabulek podmíněných pravděpodobností ve formátu, který dokáže zpracovat knihovna \texttt{catest}. % TODO: odkázat na formát Hugin flat file (aplikace Hugin Researcher .net file)
...
% TODO: proč created_at, updated_at : pro zpracování dat, proč closed_at - analýza dat, přehrávání dat, analýza délky řešení otázky apod.

% \section{Struktura aplikace}
% Schematicky je struktura zachycená na obrázku x.y. Jádro aplikace tvoří společně se základní konfigurací aplikace několik pohledových funkcí (dokumentace \textit{Flasku} je obecně uvádí jako \uv{view functions}), které definují, co se má na základě HTTP požadavku uživatele vykonat. Pro obsloužení většiny požadavků stačí, aby % TODO: obrázek

% \section{Datový model}
%
% \section{Webové rozhraní a pracovní postupy}
%
% \subsection{Rozhraní a pracovní postup studenta}
%
% \subsection{Rozhraní a pracovní postup zadavatele}
%
% \section{Analýza výpočetní náročnosti aplikace}
